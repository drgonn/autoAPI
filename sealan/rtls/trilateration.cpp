	...
	int* combination)
{
	...
	*best_gdoprate = 1; /* put the worst gdoprate init */
	...
	do {
		...
		if (success)
			cout << "Location" << ovr_r1 << ovr_r2 << ovr_r3 << ovr_r4 << "+err=" << overlook_count;
		else
			cout << "No Location" << ovr_r1 << ovr_r2 << ovr_r3 << ovr_r4 << "+err=" << overlook_count;
		...
		if (success)
		{
			...
			case TRIL_4SPHERES:
				...
				cout << "while(combination_counter)" << combination_counter;
				break;
...
int GetLocation(vec3d* best_solution, int use4thAnchor, vec3d* anchorArray, int* distanceArray)
{
    ...
    // 这个参数是判断是否有第四点，为0的时候，也就是没有第四点时候，p4等于p1
	if (use4thAnchor == 0)
	{
		r4 = r1;
    ...
    // 计算结果，返回的类型是int，所以返回的还是一个状态，同样还是把best_solution这一指针传递进去了
    // 
	...
	if (result >= 0)
	{
		...
		cout << "O1" << "x=" << o1.x << "y=" << o1.y << "z=" << o1.z;
		cout << "O2" << "x=" << o2.x << "y=" << o2.y << "z=" << o2.z;

		cout << "solution" << "x=" << best_solution->x << "y=" << best_solution->y << "z=" << best_solution->z;
		cout << "result=" << result;
		return result;
